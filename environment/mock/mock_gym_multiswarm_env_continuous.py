...
class MockContinuousPmsoGymEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        low_limit_subswarm_action_space = [-(config.w - config.w_min), -(config.c1 - config.c_min),
                                           -(config.c2 - config.c_min)]
        high_limit_subswarm_action_space = [config.w_max - config.w, config.c_max - config.c1, config.c_max - config.c2]
        low_limit_action_space = np.array(
            [low_limit_subswarm_action_space for _ in range(config.num_sub_swarms)]).flatten()
        high_limit_action_space = np.array(
            [high_limit_subswarm_action_space for _ in range(config.num_sub_swarms)]).flatten()
