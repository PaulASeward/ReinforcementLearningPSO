...
class DDRPGAgent(BaseAgent):
    ...
    def initialize_current_state(self):
        self.episode_states = np.zeros([self.config.env_config.trace_length,
                                        self.config.env_config.observation_length])  # Starts with choosing an action from empty states. Uses rolling window size 4
