...
class DRQNAgent(BaseAgent):
    ...
    def initialize_current_state(self):
        self.episode_states = np.zeros([self.config.env_config.trace_length, self.config.env_config.observation_length])  # Starts with choosing an action from empty states. Uses rolling window size 4
        observation, swarm_info = self.env.reset()
        self.update_episode_states(observation)
        return np.reshape(self.episode_states, [1, self.config.env_config.trace_length, self.config.env_config.observation_length])
