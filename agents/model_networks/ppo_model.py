...
class ActorNetworkModel(BaseModel):
    ...
    def nn_model(self):
        ...
        # output = Dense(self.rl_env_config.action_dimensions, activation=None)(output)
        log_std = LogStdLayer(self.rl_env_config.action_dimensions)(x)
        # log_std = tf.Variable(initial_value=-0.5 * tf.ones(self.rl_env_config.action_dimensions, dtype=tf.float32),
