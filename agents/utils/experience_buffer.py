...
class PPOBuffer:
    ...
    def __init__(self, config):
        self.obs_buf = np.zeros((config.env_config.num_episodes, config.env_config.observation_length), dtype=np.float32)
        self.act_buf = np.zeros((config.env_config.num_episodes, config.env_config.action_dimensions), dtype=np.float32)
