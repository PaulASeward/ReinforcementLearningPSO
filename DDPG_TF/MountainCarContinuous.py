...
# replay buffer
replay_buffer = ReplayBuffer.ReplayBuffer(1000000)  # 1M history
...
def train(verbose=1, batch_size=wandb.config.batch_size, gamma=wandb.config.gamma):
    ...
    if (len(replay_buffer) > batch_size):
        [s1, a1, r, s2, done] = replay_buffer.sample(batch_size)
...
def main():
    ...
    for episode in range(500):
        ...
        for step in range(1500):
            ...
            if (len(replay_buffer) < wandb.config.learning_start):
            ...
            replay_buffer.add((np.squeeze(state), action, reward, np.squeeze(newState), done))
